...
class FilteringIterator:
    """
    Removes all nulls from input lines and convert \x01's to tabs
    """
...
def transform(it: FilteringIterator, stdout=sys.stdout, stderr=sys.stderr):
    ...
    try:
        for i, row in enumerate(reader):
            ...
            if len(row) != target_len:
                err += 1
                print(
                    f"Incorrect length. Line {i}: length {len(row)}, should be: {target_len}",
                    file=stderr,
                )
            else:
                row_errored = False
                ...
                for j, col in enumerate(columns):
                    ...
                    if struct_sep in row[j] and named_struct is None:
                        print(
                            f"Possible struct {col_name}, in row {i} => {row[j].replace(struct_sep, '<0003>')}",
                            file=stderr,
                        )
                    if is_list:
                        out_val = [
                        ...
                        if named_struct is None:
                            if col_name == "subjects":
                                out[col_name] = [
                                    v.replace(struct_sep, ":") for v in out_val
                                ]
                            else:
