...
from transform.utils.loader import SEPARATE_COLLECTION, GUIDELINE, ADDRESS, OUTPUT_PATH
from transform.utils.send import send_data
from transform.utils.save import save_pd_df
...
def transform_guidelines(address: str) -> DataFrame:
    """Transform guidelines"""
    guidelines = requests.get(address, timeout=20).json()["results"]
...
def upload_guidelines(env_vars: dict) -> None:
    """Upload guidelines"""
    addresses = env_vars[SEPARATE_COLLECTION][GUIDELINE][ADDRESS]

    for address_num, address in enumerate(tqdm(addresses.split(" "), desc=GUIDELINE)):
        guidelines_df = transform_guidelines(address)
        save_pd_df(guidelines_df, env_vars[OUTPUT_PATH])
        send_data(env_vars, GUIDELINE, address, address_num)
