...
} from '@eosc-search-service/common';
  ...
  template: `
    ...
    >
      ...
      <div class="badge" *ngFor="let activeFilter of activeFilters$ | async">
        ...
        >
        <span class="close-btn btn-primary" (click)="removeFilter(activeFilter)"
          >x</span
        >
      </div>
    </section>
...
export class ActiveFiltersComponent implements OnInit {
  ...
  q$ = this._route.queryParamMap.pipe(map((params) => params.get('q')));
  ...
  set collections(collections: ICollectionSearchMetadata[] | null) {
    this.filtersConfigurations$.next(
      (collections || [])
        ...
        .reduce(
          (acc, filterConfigurations) => [...acc, ...filterConfigurations],
          []
        )
    );
  }
      ...
      .pipe(
        ...
        map((filtersValues) =>
          filtersValues.map(
            ([activeFilter, value]) =>
              ({
                ...this.filtersConfigurations$
                  .getValue()
                  .find(({ filter }) => filter === activeFilter),
                value,
              } as IActiveFilter)
          )
        )
