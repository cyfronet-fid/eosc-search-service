...
@Component({
  selector: 'ess-filter-checkbox-status',
  template: `
    <div class="filter">
      <ess-filter-label
        [label]="label"
...
@UntilDestroy()
export class FilterCheckboxStatusComponent implements OnInit {
  @Input()
  label!: string;
  ...
  data: Record<string, Record<string, string | boolean>> = {
    ongoing: { label: 'Ongoing', value: 'ongoing', checked: false },
    closed: { label: 'Closed', value: 'closed', checked: false },
  };
