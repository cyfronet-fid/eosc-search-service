...
import { IFilterEntry, mapFacetToTreeNodes } from './filters.model';
import {
...
export class FiltersService {
  ...
  searchFilters$(
    ...
    return this.fetchMoreForSingle$(id, activeSet.collection, params, query, fetchNext);
  }
        ...
        tap((data) => {
          this._repository.store.update(
            updateEntities(id, (f) => {
              const out = {
                ...f,
                offset: f.offset + FILTERS_FETCH_MORE_LIMIT,
                moreAvailable:
                  data.facets[id].buckets.length === FILTERS_FETCH_MORE_LIMIT,
                data: fetchNext
                  ? [...f.data, ...mapFacetToTreeNodes(data.facets[id], id)]
                  : mapFacetToTreeNodes(data.facets[id], id),
              };
              return out;
            })
          );
